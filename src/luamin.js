...
function CreateLuaTokenStream(text) {
  ...
  while (true) {
      ...
      const leadingWhite = text.substr(whiteStart, (p - whiteStart))
      ...
      if (c1 == '') {
          ...
          while (true) {
              ...
              if (c2 == '\\') {
                  const c3 = get()
                  const esc = CharacterForEscape[c3]
                  if (esc == null) {
...
function CreateLuaParser(text) {
  ...
  const tokens = CreateLuaTokenStream(text)
  ...
  function expect(type, source) {
      ...
      if (tk.Type == type && (source == null || tk.Source == source)) {
          ...
          if (source) {
              const a = `${getTokenStartPosition(tk)}: \`${source}\` expected.`
              throw a
          } else {
              const a = `${getTokenStartPosition(tk)}: ${type} expected.`
              throw a
          }
      }
  }
  ...
  function MkNode(node) {
      const getf = node.GetFirstToken
      const getl = node.GetLastToken
  ...
  function exprlist(locals, upvals) {
      const exprList = [expr(locals, upvals)]
      const commaList = []
      while (peek().Source == ",") {
          commaList.push(get())
  ...
  function prefixexpr(locals, upvals) {
      ...
      if (tk.Source == '(') {
          const oparenTk = get()
          const inner = expr(locals, upvals)
          ...
          print(debugMark())
          const a = (`${getTokenStartPosition(tk)}: Unexpected symbol. ${tk.Type} ${tk.Source}`)
          throw a
      }
  }
  ...
  function tableexpr(locals, upvals) {
      ...
      while (peek().Source != "}") {
          ...
          if (peek().Source == '[') {
              // Index
              const obrac = get()
              const index = expr(locals, upvals)
              const cbrac = expect("Symbol", "]")
              const eq = expect("Symbol", "=")
              const value = expr(locals, upvals)
  ...
  function primaryexpr(locals, upvals) {
      ...
      while (true) {
          ...
          if (tk.Source == ".") {
              ...
              const colonTk = get()
              const methodName = expect("Ident")
              const fargs = functionargs(locals, upvals)
              let node
              node = MkNode({
                  "Type": "MethodExpr",
                  "Base": base,
              ...
              const compoundTk = get()
              const rhsExpr = expr(locals, upvals)
  ...
  function subexpr(limit, locals, upvals) {
      ...
      if (isUnop()) {
          const opTk = get()
          const ex = subexpr(UnaryPriority, locals, upvals)
          let node
          node = MkNode({
              "Type": "UnopExpr",
              "Token_Op": opTk,
      ...
      while (isBinop() && BinaryPriority[peek().Source] != undefined && BinaryPriority[peek().Source][0] > limit) {
          const opTk = get()
          const rhs = subexpr(BinaryPriority[opTk.Source][1], locals, upvals)
  ...
  function exprstat(locals, upvals) {
      const ex = primaryexpr(locals, upvals)
      ...
      if (ex.Type == "MethodExpr" || ex.Type == "CallExpr") {
          ...
          while (peek().Source == ",") {
              ...
              const lhsPart = primaryexpr(locals, upvals)
  ...
  function ifstat(locals, upvals) {
      const ifKw = get()
      const condition = expr(locals, upvals)
      const thenKw = expect("Keyword", "then")
      const ifBody = block(locals, upvals)
      const elseClauses = []
      while (peek().Source == "elseif" || peek().Source == "else") {
          ...
          const elseifBody = block(locals, upvals)
          elseClauses.push({
  ...
  function dostat(locals, upvals) {
      const doKw = get()
      const [body, enKw] = blockbody("end", locals, upvals)
  ...
  function forstat(locals, upvals) {
      const forKw = get()
      const [loopVars, loopVarCommas] = varlist()
      const node = []
      if (peek().Source == "=") {
          const eqTk = get()
          const [exprList, exprCommaList] = exprlist(locals, upvals)
          if (exprList.length < 2 || exprList.length > 3) {
              throw "Expected 2 or 3 values for range bounds"
          }
          const doTk = expect("Keyword", "do")
          const [body, enTk] = blockbody("end", locals, upvals)
          let node
          node = MkNode({
              "Type": "NumericForStat",
              "VarList": loopVars,
  ...
  function repeatstat(locals, upvals) {
      const repeatKw = get()
      const [body, untilTk] = blockbody("until", locals)
      const condition = expr(locals, upvals)
  ...
  function localdecl(locals, upvals) {
      ...
      if (peek().Source == "function") {
          const funcStat = funcdecl(false, locals, upvals, true)
          ...
          const [varList, varCommaList ] = varlist(false, true)
          let exprList = []
          let exprCommaList = []
          let eqToken
          if (peek().Source == "=") {
              eqToken = get()
              const [exprList1, exprCommaList1] = exprlist(locals, upvals)
              exprList = exprList1
  ...
  block = function(a, b) {
      ...
      while (!isLast && !isBlockFollow()) {
          ...
          thing = peek()
          const [isLast, stat] = statement(locals, upvals)
          if (stat) {
...
function VisitAst(ast, visitors) {
  ...
  function preVisit(exprOrStat) {
      if (exprOrStat != null) {
          const visitor = visitors[exprOrStat.Type]
...
function AddVariableInfo(ast) {
  ...
  function getLocalVar(scope, name) {
      ...
      if (scope.ParentScope) {
          const _var = getLocalVar(scope.ParentScope, name)
          if (_var) {
              return _var
          }
      }
  ...
  function referenceVariable(name, setNameFunc) {
      ...
      const curLocation = markLocation()
  ...
  visitor.FunctionLiteral = {
      ...
      "Pre": function(expr) {
          ...
          expr.ArgList.forEach((ident, index) => {
              const _var = addLocalVar(ident.Source, function(name, d) {
                  if (!d)
                      ident.Source = name
              }, {
                  "Type": "Argument",
...
function PrintAst(ast) {
  ...
  printStat = function(stat) {
      ...
      if (stat.WrapInDo) {
          const wspace = getLeadingWhite(stat.GetFirstToken())
  ...
  function applyIndent(token) {
      ...
      if (token.LeadingWhite == '' || (token.LeadingWhite.substr(-indentString.length, indentString.length) != indentString)) {
          ...
          for (i=token.LeadingWhite.length; i>=0; i--) {
              const cur = token.LeadingWhite.substr(i, 1)
...
function StripAst(ast) {
  ...
  function joint(tokenA, tokenB, shit = false) {
      ...
      const lastCh = (typeof tokenA.Source == 'string' ? tokenA.Source : tokenA.Source.toString()).substr(tokenA.Source.length - 1,1)
      const firstCh = (typeof tokenB.Source == 'string' ? tokenB.Source : tokenB.Source.toString()).substr(0,1)
  ...
  function bodyjoint(open, body, close) {
      stripStat(body)
      stript(close)
      const bodyFirst = body.GetFirstToken()
      const bodyLast = body.GetLastToken()
...
function SolveMath(ast) { // This is some ugly code sorry for whoever is seeing this
  ...
  function solvebinop(operator, left1, right1) {
      ...
      const a = (lhs.Token) || (lhs.Expression != null && lhs.Expression.Token) || null
      const b = (rhs.Token) || (rhs.Expression != null && rhs.Expression.Token) || null

      //if (a == null || b == null) return;
      //if (a.Source == null || b.Source == null) return;

      const lSrc = a != null ? a.Source : null
      const rSrc = b != null ? b.Source : null
  ...
  function solveunop(operator, rhs) {
      const b = rhs.Token || rhs.Expression || rhs.EntryList || rhs
      ...
      if (rhs.Type == "TableLiteral" && b != null) {
              ...
              if (ignoreRest) {
                  ...
                  if (v.EntryType == "Value" || v.EntryType == "Index") {
                      if ((v.Index == null || v.Index.Type == "NumberLiteral") && v.Value) {
                          ...
                          const index = (v.Index != null && v.Index.Token != null && v.Index.Token.Source !== null) ? (v.Index.Token.Source) : lastIndex + 1
          ...
          if (operator == "#") {
              ...
              const newex = createbinop("+", createtype("NumberLiteral", amount), createunop("#", rhs));
              return newex
          }
      }
  ...
  solveExpr = function(expr) {
      if (expr.Type == "BinopExpr") {
          ...
          if (expr.Lhs != null && expr.Rhs != null) {
              const tokenOp = expr.Token_Op

              if (tokenOp != null && tokenOp.Source != null) {
                  const val = solvebinop(tokenOp.Source, expr.Lhs, expr.Rhs)

                  if (val != null) {
                      if (typeof(val) == "boolean") {
                          const b = createtype("BooleanLiteral", val.toString(), "Keyword")
                          replace(expr, b)
                          return
                      } else if(typeof(val) == "number") {
                          if (isFinite(val) == true) {
                              const num = createtype("NumberLiteral", val.toString(), "Number")
                              replace(expr, num)
                              return
                          }
                      } else if(typeof(val) == "string") {
                          const str = createtype("StringLiteral", val, "String")
                          replace(expr, str)
              ...
              if (expr.Lhs.Type == "ParenExpr") {
                  const exprt = expr.Lhs
                  const expression = exprt.Expression
          ...
          if (expr.Rhs != null && canSolve[expr.Rhs.Type] == true) {
              ...
              if (tokenOp != null && tokenOp.Source != null) {
                  const rhs = expr.Rhs.Expression != null ? expr.Rhs.Expression : expr.Rhs
                  const val = solveunop(tokenOp.Source, rhs)
                  ...
                  if (val != null) {
                      if (typeof(val) == "boolean") {
                          ...
                          if (isFinite(val) == true) {
                              const num = createtype("NumberLiteral", val, "Number")
                              replace(expr, num)
                              return
                          }
                      } else if(typeof(val) == "string") {
              ...
              || expr.Type == "VargLiteral" || expr.Type == 'HashLiteral')
      {
          ...
          if (token != null) {
              if (token.Type == "Number") {
                  const int = token.Source.toString().split('e')
                  if (int.length === 2) {
                      const l = parseFloat(int[0])
                      const r = parseFloat(int[1])
              ...
              if (token.Type == "String") {
                  token.Source = token.Source.replace(/\\\d+/gi, (got) => {
                      const num = parseFloat(got.substr(1,got.length-1))
          ...
          {
              const fLit = expr.Base.Expression
              expr.FunctionArguments.ArgList.forEach((v, i) => {
                  const c = expr.FunctionArguments.ArgList[i]
          ...
          const exprExpr = expr.Expression
  ...
  solveStat = function(stat) {
      if (stat.Type == "StatList") {
          ...
          const a = stat.RangeList[0]
          const b = stat.RangeList[1]
          const c = stat.RangeList[2]
          if (a == null || b == null) {
              return stat.Remove()
          }
          ...
          const start = parseFloat(a.Token.Source)
          const end = parseFloat(b.Token.Source)
          const step = (c != null && parseFloat(c.Token.Source)) || 1

          const t1 = ((step > 0 && start <= end) || (step < 0 && start >= end))
          const t2 = ((end - start) + step) / step

          const willRun = t1 && t2 >= 0
          ...
          if (stat.Body === null || (stat.Body.Type == "StatList" && stat.Body.StatementList.length === 0)) {
              ...
              const s = stat.Body.StatementList[0]
...
function indexToVarName(index) {
  let id = ""
  const d = index % VarStartDigits.length
  index = (index - d) / VarStartDigits.length
  id = `${id}${VarStartDigits[d]}`
  while (index > 0) {
      const d = index % VarDigits.length
...
function SolveCFlow(ast) {
  ...
  visitor.WhileStat = stat => {
      ...
      const enums = []
      const enumIndex = []
      for (const statV of Object.entries(stat.Body.StatementList)) {
          if (statV[1].Type !== 'IfStat' || statV[1].ElseClauseList.length !== 0)
              return

          const condition = GetInnerExpression(statV[1].Condition)
          ...
          if (condition.Type === 'BinopExpr') {
              ...
              let nextEnum = null
              for (const stat2V of Object.entries(statV[1].Body.StatementList)) {
      ...
      const order = [ ]
      const order2 = []
      for (const obj of Object.entries(enums)) {
          const V = obj[1]
          if (V.nextEnum === null) {
              order.push(V.enum)
              continue
          ...
          const index = order.indexOf(V.nextEnum)
          const index2 = order2.indexOf(V.Enum)
          if (index !== -1) {
...
luaminp.Minify = function(scr, options) {
  ...
  const [glb, root] = AddVariableInfo(ast)
